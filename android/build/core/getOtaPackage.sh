...
cp -u out/host/linux-x86/bin/minigzip  out/host/linux-x86/bin/mkbootfs  out/host/linux-x86/bin/mkbootimg  out/host/linux-x86/bin/fs_config  out/host/linux-x86/bin/mkyaffs2image  out/host/linux-x86/bin/zipalign  out/host/linux-x86/bin/aapt  out/host/linux-x86/bin/bsdiff  out/host/linux-x86/bin/imgdiff  out/host/linux-x86/bin/mkuserimg.sh  out/host/linux-x86/bin/make_ext4fs  out/host/linux-x86/bin/genext2fs  $ROOTPATH/out/host/linux-x86/bin/
...
#mediatek
#mkdir -p $ROOTPATH/mediatek/source/misc
#cp -u mediatek/source/misc/ota_scatter.txt  $ROOTPATH/mediatek/source/misc/
#ota.zip
#cp -u out/target/product/$1/*.zip $ROOTPATH/org.zip
#org.zip
...
zip -rq otaPackage.zip bootable build mediatek out configure.xml build.prop ota_"${buildnumber/ /_}".zip #org_$buildnumber.zip
cd ..
#mv otaPackage/otaPackage.zip "${product/ /_}"-ota-$buildnumber.zip
mv otaPackage/otaPackage.zip $1/adups-otaPackage.zip
rm -rf otaPackage

#检查MODEM侧几个文件是否存在，不存在侧删除adups-otaPackage.zip -_-!
if [ ! -f modem_images/emmc_appsboothdtail.mbn ]; then
echo "ERR: emmc_appsboothdtail.mbn do not exist ,delete file:$1/adups-otaPackage.zip"
if [ -f $1/adups-otaPackage.zip ]; then
rm -f $1/adups-otaPackage.zip
fi
fi

if [ ! -f modem_images/fat.bin ]; then
echo "ERR: fat.bin do not exist ,delete file:$1/adups-otaPackage.zip"
if [ -f $1/adups-otaPackage.zip ]; then
rm -f $1/adups-otaPackage.zip
fi
fi

if [ ! -f modem_images/oemsbl.mbn ]; then
echo "ERR: oemsbl.mbn do not exist ,delete file:$1/adups-otaPackage.zip"
if [ -f $1/adups-otaPackage.zip ]; then
rm -f $1/adups-otaPackage.zip
fi
fi

if [ ! -f modem_images/oemsblhd.mbn ]; then
echo "ERR: oemsblhd.mbn do not exist ,delete file:$1/adups-otaPackage.zip"
if [ -f $1/adups-otaPackage.zip ]; then
rm -f $1/adups-otaPackage.zip
fi
fi

if [ ! -f modem_images/oemsblhdtail.mbn ]; then
echo "ERR: oemsblhdtail.mbn do not exist ,delete file:$1/adups-otaPackage.zip"
if [ -f $1/adups-otaPackage.zip ]; then
rm -f $1/adups-otaPackage.zip
fi
fi

if [ ! -f modem_images/qcsblhd_cfgdata.mbn ]; then
echo "ERR: qcsblhd_cfgdata.mbn do not exist ,delete file:$1/adups-otaPackage.zip"
if [ -f $1/adups-otaPackage.zip ]; then
rm -f $1/adups-otaPackage.zip
fi
fi

if [ ! -f modem_images/qcsbl.mbn ]; then
echo "ERR: qcsbl.mbn do not exist ,delete file:$1/adups-otaPackage.zip"
if [ -f $1/adups-otaPackage.zip ]; then
rm -f $1/adups-otaPackage.zip
fi
fi
