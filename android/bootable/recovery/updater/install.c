...
extern Value* PackageExtractFileFnAdupsFota(const char* name, State* state, int argc, Expr* argv[]);
extern Value* ApplyPatchFnAdupsFota(const char* name, State* state, int argc, Expr* argv[]);
#endif
//adupsfota end
...
//adupsfota start
#ifdef ADUPS_FOTA_SUPPORT
    RegisterFunction("package_extract_file", PackageExtractFileFnAdupsFota);
#else
...
//adupsfota start
#ifdef ADUPS_FOTA_SUPPORT
    RegisterFunction("apply_patch", ApplyPatchFnAdupsFota);
#else
